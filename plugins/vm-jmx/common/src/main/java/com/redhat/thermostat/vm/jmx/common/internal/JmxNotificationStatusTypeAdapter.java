...
public class JmxNotificationStatusTypeAdapter extends TypeAdapter<JmxNotificationStatus> {
    ...
    @Override
    public JmxNotificationStatus read(JsonReader in) throws IOException {
        throw new UnsupportedOperationException();
    }

    @Override
    public void write(JsonWriter out, JmxNotificationStatus status) throws IOException {
        out.beginObject();
        out.name(TIMESTAMP);
        writeLong(out, status.getTimeStamp());
        out.name(ENABLED);
        out.value(status.isEnabled());
        out.name(VM_ID);
