...
public class VmClassloaderInfoExtractorTest {

    private VmClassloaderInfoExtractor extractor;
    private VmUpdate update;

    @Before
    public void setup() {
        update = mock(VmUpdate.class);
        extractor = new VmClassloaderInfoExtractor(update);
    }

    @Test
    public void testClassesCount() throws VmUpdateException {
        final String MONITOR_NAME = "java.cls.loadedClasses";
        final Long MONITOR_VALUE = 300l;

        when(update.getPerformanceCounterLong(eq(MONITOR_NAME))).thenReturn(MONITOR_VALUE);

        Long returned = extractor.getLoadedClassesCount();
